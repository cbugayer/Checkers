...
TEST_CASE("Find_moves check")
{
    ...
    CHECK_FALSE(m.is_game_over());
    CHECK(m.turn() == Player::dark);
    //check starting possible moves for standard pieces: find_move
    CHECK(m[{2, 2}] == Player::dark);
    CHECK(m.check_king({2,2}) == Piece::standard);
    const Move* myfirstmove =  m.find_move({2, 2});
    CHECK(myfirstmove->second == Position_set{{1,3}, {3,3}});
    //actual plays
    m.play_move({0, 2}, {1,3});
    m.play_move({1, 5}, {2,4});
    m.play_move({2, 2}, {3,3});
    ...
    CHECK(m.check_king({2,0}) == Piece::king);
    m.play_move({1,1}, {2,2});
    m.play_move({2,0}, {3,1});
    m.play_move({2,2}, {1,3});
    m.play_move({0,4}, {2,2});
    m.play_move({7,1}, {6,2});
    //check possible moves for king pieces: find_move
    const Move* king_moves_one =  m.find_move({3, 1});
    CHECK(king_moves_one->second == Position_set{{2,0},
                                                 {4,0},
                                                 {4,2}});
    //actual moves
    m.play_move({3,1}, {4,2});
    ...
    const Move* king_moves_two =  m.find_move({4, 2});
    CHECK(king_moves_two->second == Position_set{{3,1},
                                                 {5,1},
                                                 {3,3},
                                                 {5,3}});
...
TEST_CASE("check scores")
{
    ...
    m.play_move({3,5},{2,4}); //light
    CHECK(m.get_dark_score() == 0);
    m.play_move({1,3},{3,5}); //dark
    CHECK(m.get_dark_score() == 1);
    CHECK(m.get_light_score() == 0);
    m.play_move({4,6},{2,4}); //light
    CHECK(m.get_light_score() == 1);
    m.play_move({6,2},{7,3}); //dark
    m.play_move({2,6},{3,5}); //light
    m.play_move({7,1},{6,2}); //dark
    m.play_move({2,4},{3,3}); //light
    CHECK(m.turn() == Player::dark);
    m.play_move({2,2},{4,4}); //dark jump
    CHECK(m.turn() == Player::dark);
    CHECK(m.get_dark_score() == 2);
    m.play_move({4,4},{2,6}); //dark double jump
    CHECK(m.get_dark_score() == 3);
...
TEST_CASE("Draw: Count exceeds 25")
{
    ...
    CHECK(m.check_king({3,7}) == Piece::king);
