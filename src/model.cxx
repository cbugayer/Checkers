...
Model::compute_next_moves_()
{
    ...
    for (auto a_pos : board_.all_positions()) {
        if (check_for_j(board_2(static_cast<Position>(a_pos)),
                        static_cast<Position>(a_pos))) {
            a_pset = evaluate_position_(static_cast<Position>(a_pos),
                                        turn(),
                                        board_2(static_cast<Position>
                                        (a_pos)));
...
Piece
Model::check_king(Position p)
{
    if (board_2(p) == Piece::king){
        return Piece::king;
    }else if (board_2(p) == Piece::standard){
