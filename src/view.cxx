...
void View::draw(Sprite_set& set, Position mp) {
    ...
    if (model_.highlighted().empty()) {
        for (auto p: model_.board()) {
            ...
            if (pset[p]) {
                set.add_sprite(highlighting_sprite,
                               {static_cast<double> (p.x * (dark_square
                               .dimensions()
                                                  .width * 1.0)),
                                static_cast<double>(p.y * (dark_square
                                .dimensions()
                                                  .height * 1.0))},
                               10);
            }
        ...
        for (auto p: model_.highlighted()) {
            set.add_sprite(highlighting_sprite,
                           {(double)p.x * (dark_square.dimensions()
                                              .width * 1.0),
                            (double)p.y * (dark_square.dimensions()
                                              .height * 1.0)},
                           10);
        }
    }
    for (int i = 0; i < model_.board().dimensions().width; i += 2) {
        for (int j = 0; j < model_.board().dimensions().height; j += 2) {
            ...
            set.add_sprite(dark_square,
                           {(double)i * (dark_square.dimensions().width
                           * 1.0),
                            (double)j * (dark_square.dimensions().height
                            * 1.0)},
                           0);
    ...
    for (int i = 0; i < model_.board().dimensions().width; i += 2) {
        for (int j = 1; j < model_.board().dimensions().height; j += 2) {
            ...
            set.add_sprite(light_square,
                           {(double)i * (light_square.dimensions().width
                           * 1.0),
                            (double)j * (light_square.dimensions().height
                            * 1.0)},
                           0);
    ...
    for (int i = 1; i < model_.board().dimensions().width; i += 2) {
        for (int j = 0; j < model_.board().dimensions().height; j += 2) {
            ...
            set.add_sprite(light_square,
                           {(double)i * (light_square.dimensions()
                           .width * 1.0),
                            (double)j * (light_square.dimensions().height
                            * 1.0)},
                           0);
    ...
    dark.color(ge211::Color::black()) << std::to_string
    (model_.get_dark_score());
    ...
    light.color(ge211::Color(red_piece_color)) << std::to_string
    (model_.get_light_score());
        ...
        Position pos) const
{
    ...
    if (model_[board_pos] == Player::dark) {
        if (model_2(board_pos) == Piece::standard) {
            ...
            set.add_sprite(black_piece,
                           {(double)1.0 * pos.x, (double)1.0 * pos.y},
                           100);
            set.add_sprite(crown,
                           {(double)1.0 * pos.x + 7.0,
                            (double)1.0 * pos.y + 13.0},
                           110,
